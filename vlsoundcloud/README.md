...
По умолчанию в плеере подключён **`assets/animations/web/chrome_cubes_portrait.html`** — WebGL/Three.js демо с кубами (загружается в `WebView`, для three.js нужен интернет). Боковая панель с ползунками в приложении скрывается скриптом, остаётся canvas.
